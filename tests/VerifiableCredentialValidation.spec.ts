...
import { TokenType, IExpectedVerifiableCredential, Validator } from '../lib';
 ...
 describe('VerifiableCredentialValidation', () => {
  ...
  it('should test validate', async () => {
    ...
    let response = await validator.validate(siop.vc.rawToken, setup.defaultUserDid, Validator.readContractId(siop.contract));
    expect(response.result).toBeTruthy();

    // Negative cases

    // Bad VC signature
    response = await validator.validate(siop.vc.rawToken + 'a', setup.defaultUserDid, Validator.readContractId(siop.contract));
    ...
    response = await validator.validate(token.rawToken, setup.defaultUserDid, Validator.readContractId(siop.contract));
    expect(response.result).toBeFalsy();
    expect(response.status).toEqual(403);
