...
import { PortableIdentityCardError } from "../../lib/error-handling/PortableIdentityCardError";
...
describe('ContractValidator', () => {
  ...
  describe("validate()", () => {
    ...
    it("should validate contract", () => {
      const actualValidContract = validator.validate(validContract)
      expect(validContract).toEqual(actualValidContract)
    });
    
    it("should throw invalid contract error - no display and input", () => {
      try {
        validator.validate(invalidContractNoDisplayAndInput)
        fail()
      } catch (exception) {
        expect(exception).toBeInstanceOf(PortableIdentityCardError)
      }
    });
    
    it("should throw invalid contract error - no contract", () => {
      try {
        validator.validate(invalidContractNoContract)
        fail()
      } catch (exception) {
        expect(exception).toBeInstanceOf(PortableIdentityCardError)
      }
